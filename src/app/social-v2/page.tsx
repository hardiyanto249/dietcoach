    ...
    const handleSaveEdit = async (postId: string, commentId: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const error = await res.json();
                alert(`Gagal mengedit komentar: ${error.error || 'Unknown error'}`);
            }
        } catch (error) {
            console.error(error);
    ...
    const handleDeleteComment = async (postId: string, commentId: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const error = await res.json();
                alert(`Gagal menghapus komentar: ${error.error || 'Unknown error'}`);
            }
        } catch (error) {
            console.error(error);
            ...
            <div className="max-w-2xl mx-auto">
                <div className="border-b border-gray-800 p-4">
                    <div className="flex gap-3">
                        {userProfile?.email ? (
                            <div
                                className="w-12 h-12 rounded-full flex-shrink-0 ring-2 ring-purple-500/30 overflow-hidden"
                            ...
                            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-lg font-bold flex-shrink-0 ring-2 ring-purple-500/30">
                        ...
                        <div className="flex-1">
                            <textarea
                                ...
                                className="w-full bg-transparent text-base text-white resize-none outline-none placeholder-gray-600"
                            />
                                            ...
                                            <div className="mt-4 space-y-3">
                                                {loadingComments.has(post.id) ? (
                                                    ...
                                                    postComments[post.id].map(c => (
                                                        <div key={c.id} className="group border border-gray-800 rounded-2xl p-4 bg-gray-900/40 hover:bg-gray-900/60 hover:border-gray-700 transition-all">
                                                            <div className="flex gap-3">
                                                                {c.userEmail ? (
                                                                    <div
                                                                        className="w-12 h-12 rounded-full flex-shrink-0 ring-2 ring-gray-700 overflow-hidden"
                                                                        style={{ width: '48px', height: '48px', minWidth: '48px' }}
                                                                    >
                                                                        <img
                                                                    ...
                                                                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-sm font-bold flex-shrink-0 ring-2 ring-gray-700">
                                                                        {c.userName[0].toUpperCase()}
                                                                    </div>
                                                                )}
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="flex items-center justify-between mb-2">
                                                                        <p className="font-semibold text-base">{c.userName}</p>
                                                                        {userProfile?.id === c.userId && (
                                                                    ...
                                                                    {editingCommentId === c.id ? (
                                                                        <div className="space-y-2">
                                                                            <textarea
                                                                                ...
                                                                                className="w-full bg-black border border-gray-700 rounded-lg px-3 py-2 text-sm text-white resize-none outline-none focus:border-purple-500"
                                                                                rows={2}
                                                                            />
                                                                            <div className="flex gap-2">
                                                                                <button
                                                                                    onClick={() => handleSaveEdit(post.id, c.id)}
                                                                                ...
                                                                                <button
                                                                                    ...
                                                                                    className="px-3 py-1 hover:bg-gray-800 rounded-lg text-xs font-semibold transition"
                                                                                >
                                                                                    Cancel
                                                                                </button>
                                                                            </div>
                                                                        ...
                                                                        <>
                                                                            <p className="text-base leading-relaxed">{c.content}</p>
                                                                            <button
                                                                                onClick={() => handleReplyToComment(c.id, c.userName, post.id)}
                                                                                className="mt-2 text-sm text-gray-400 hover:text-purple-400 font-medium transition"
                                                                            >
                                                                                Reply
                                                                            </button>
                                                                        </>
